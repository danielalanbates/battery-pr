...
// Enforce ceiling: if battery is charging above the ceiling, re-run maintain
const enforce_ceiling = async () => {
    try {
        const { percentage, charging } = await get_battery_status()
        const { ceiling_percentage } = get_maintenance_settings()
        if( charging && Number( percentage ) >= ceiling_percentage ) {
            log( `Ceiling enforcement: ${ percentage }% >= ${ ceiling_percentage }%, re-enabling limiter` )
            await enable_battery_limiter()
        }
    } catch ( e ) {
        log( `Error in enforce_ceiling: `, e )
    }
}
